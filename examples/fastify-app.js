/**
 * AuthSnap — Fastify Example (All 6 Providers)
 *
 * Setup:
 *   1. Copy .env.example to .env and fill in your OAuth credentials
 *   2. Run: node examples/fastify-app.js
...
// Register cookie plugin (required for AuthSnap state cookies)
...
// ─── 3. Protect routes ──────────────────────────────────────────────
fastify.get('/dashboard', { preHandler: auth.protect() }, (request, reply) => {
  reply.type('text/html').send(`
    <html>
      ...
      <body style="font-family: system-ui, sans-serif; max-width: 640px; margin: 60px auto;">
        ...
        <pre style="background: #f4f4f4; padding: 16px; border-radius: 8px; overflow-x: auto;">${JSON.stringify(request.user, null, 2)}</pre>
...
console.log(`\nAuthSnap Fastify example running at http://localhost:${PORT}`);
